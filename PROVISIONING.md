# How to provision a new environment for Lola
...
* Enter domains to secure (for example): `lola.xn--facebok-i0a.com`
* Enter webroot path: `/home/deployer/apps/Lola/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/lola.xn--facebok-i0a.com/fullchain.pem /etc/ssl/Lola.crt
ln -s /etc/letsencrypt/live/lola.xn--facebok-i0a.com/privkey.pem /etc/ssl/Lola.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/Lola/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
43 3,15 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/Lola/shared/public/
```

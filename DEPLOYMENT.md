...
sudo systemctl <start|stop|restart|status> Lola-puma.service
sudo systemctl <start|stop|restart|status> Lola-sidekiq.service
```

Note that the `deployer` user has permission to restart `Lola-puma.service` and `Lola-sidekiq.service`, but nothing else. All other `systemctl` commands require `root`.
...
```
/home/deployer/apps/Lola/shared/.env.production
```

After making changes to this file, be sure to restart the application:

```
sudo systemctl restart Lola-puma.service
sudo systemctl restart Lola-sidekiq.service
```
